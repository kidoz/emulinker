...
public class GameStatus extends V086Message {
    ...
    public GameStatus(int messageNumber, int gameID, int val1, byte gameStatus, byte numPlayers,
            byte maxPlayers) throws MessageFormatException {
        super(messageNumber);
        ...
        if (gameStatus < 0 || gameStatus > 2)
            throw new MessageFormatException("Invalid " + getDescription()
                    + " format: gameStatus out of acceptable range: " + gameStatus);

        if (numPlayers < 0 || numPlayers > 0xFF)
            throw new MessageFormatException("Invalid " + getDescription()
                    + " format: numPlayers out of acceptable range: " + numPlayers);

        if (maxPlayers < 0 || maxPlayers > 0xFF)
            throw new MessageFormatException("Invalid " + getDescription()
                    + " format: maxPlayers out of acceptable range: " + maxPlayers);
    ...
    public String toString() {
        return getInfoString() + "[gameID=" + gameID + " gameStatus="
                + su.kidoz.kaillera.model.KailleraGame.STATUS_NAMES[gameStatus] + " numPlayers="
                + numPlayers + " maxPlayers=" + maxPlayers + "]";
    }
